 ...
 *
 * @page rt_test_sequence_007 [7] Mutexes, Condition Variables and Priority Inheritance
 *
 ...
 * - @subpage rt_test_007_008
 * - @subpage rt_test_007_009
 * .
 */
...
/**
 * @page rt_test_007_001 [7.1] Priority enqueuing test
 *
 * <h2>Description</h2>
 ...
 * - [7.1.2] Locking the mutex.
 * - [7.1.3] Five threads are created that try to lock and unlock the
 *   mutex then terminate. The threads are created in ascending
 *   priority order.
 * - [7.1.4] Unlocking the mutex, the threads will wakeup in priority
...
#if (CH_DBG_THREADS_PROFILING) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_002 [7.2] Priority inheritance, simple case
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.2.1] Getting the system time for test duration measurement.
 * - [7.2.2] The three contenders threads are created and let run
 *   atomically, the goals sequence is tested, the threads must
 *   complete in priority order.
 * - [7.2.3] Testing that all threads completed within the specified
...
#if (CH_DBG_THREADS_PROFILING) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_003 [7.3] Priority inheritance, complex case
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.3.1] Getting the system time for test duration measurement.
 * - [7.3.2] The five contenders threads are created and let run
 *   atomically, the goals sequence is tested, the threads must
 *   complete in priority order.
 * - [7.3.3] Testing that all threads completed within the specified
...
/**
 * @page rt_test_007_004 [7.4] Priority return verification
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.4.1] Getting current thread priority P(0) and assigning to the
 *   threads A and B priorities +1 and +2.
 * - [7.4.2] Spawning threads A and B at priorities P(A) and P(B).
 * - [7.4.3] Locking the mutex M1 before thread A has a chance to lock
 *   it. The priority must not change because A has not yet reached
 *   chMtxLock(M1). the mutex is not locked.
 * - [7.4.4] Waiting 100mS, this makes thread A reach chMtxLock(M1) and
 *   get the mutex. This must boost the priority of the current thread
 *   at the same level of thread A.
 * - [7.4.5] Locking the mutex M2 before thread B has a chance to lock
 *   it. The priority must not change because B has not yet reached
 *   chMtxLock(M2). the mutex is not locked.
 * - [7.4.6] Waiting 100mS, this makes thread B reach chMtxLock(M2) and
 *   get the mutex. This must boost the priority of the current thread
 *   at the same level of thread B.
 * - [7.4.7] Unlocking M2, the priority should fall back to P(A).
 * - [7.4.8] Unlocking M1, the priority should fall back to P(0).
 * .
 */
...
/**
 * @page rt_test_007_005 [7.5] Repeated locks, non recursive scenario
 *
 * <h2>Description</h2>
 ...
 *   it is not owned.
 * - [7.5.3] Locking the mutex second time, it must fail because it is
 *   already owned.
 * - [7.5.4] Unlocking the mutex then it must not be owned anymore and
 *   the queue must be empty.
 * - [7.5.5] Testing that priority has not changed after operations.
 * - [7.5.6] Testing chMtxUnlockAll() behavior.
 * - [7.5.7] Testing that priority has not changed after operations.
 * .
 */
...
#if (CH_CFG_USE_MUTEXES_RECURSIVE) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_006 [7.6] Repeated locks using, recursive scenario
 *
 * <h2>Description</h2>
 * The behavior of multiple mutex locks from the same thread is tested
 ...
 * - [7.6.2] Locking the mutex first time, it must be possible because
 *   it is not owned.
 * - [7.6.3] Locking the mutex second time, it must be possible because
 *   it is recursive.
 * - [7.6.4] Unlocking the mutex then it must be still owned because
 *   recursivity.
 * - [7.6.5] Unlocking the mutex then it must not be owned anymore and
 *   the queue must be empty.
 * - [7.6.6] Testing that priority has not changed after operations.
 * - [7.6.7] Testing consecutive chMtxTryLock()/chMtxTryLockS() calls
 *   and a final chMtxUnlockAllS().
 * - [7.6.8] Testing consecutive chMtxLock()/chMtxLockS() calls and a
 *   final chMtxUnlockAll().
 * - [7.6.9] Testing that priority has not changed after operations.
 * .
 */

static void rt_test_007_006_setup(void) {
  chMtxObjectInit(&m1);
}
...
#if (CH_CFG_USE_CONDVARS) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_007 [7.7] Condition Variable signal test
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.7.1] Starting the five threads with increasing priority, the
 *   threads will queue on the condition variable.
 * - [7.7.2] Atomically signaling the condition variable five times
...
#if (CH_CFG_USE_CONDVARS) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_008 [7.8] Condition Variable broadcast test
 *
 * <h2>Description</h2>
 * Five threads take a mutex and then enter a conditional variable
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.8.1] Starting the five threads with increasing priority, the
 *   threads will queue on the condition variable.
 * - [7.8.2] Broarcasting on the condition variable then waiting for
...
static const testcase_t rt_test_007_008 = {
  "Condition Variable broadcast test",
  rt_test_007_008_setup,
  NULL,
  rt_test_007_008_execute
};
#endif /* CH_CFG_USE_CONDVARS */

#if (CH_CFG_USE_CONDVARS) || defined(__DOXYGEN__)
/**
 * @page rt_test_007_009 [7.9] Condition Variable priority boost test
 *
 * <h2>Description</h2>
 ...
 *
 * <h2>Test Steps</h2>
 * - [7.9.1] Reading current base priority.
...
static const testcase_t rt_test_007_009 = {
  "Condition Variable priority boost test",
  rt_test_007_009_setup,
  NULL,
  rt_test_007_009_execute
};
#endif /* CH_CFG_USE_CONDVARS */
